...
redis_password=$(prompt_user "Enter the Redis password" "63796379")
allow_host=$(prompt_user "Enter the allowed host" "0.0.0.0")

# Generate the .env file
echo "Creating .env file for Redis setup..."
cat > .env <<EOL
...
if [ -f "docker-compose.example.yaml" ]; then
    sed -e "s/\${CONTAINER_NAME}/$container_name/g" \
        ...
        -e "s/\${REDIS_COMMANDER_PORT}/$redis_commander_port/g" \
...
echo "Starting Docker Compose with --build for Redis Commander..."
if docker-compose up -d --build; then
    echo "Checking container status..."
    if [ "$(docker inspect -f '{{.State.Running}}' $container_name)" = "true" ]; then
        echo "Redis Commander setup is complete and running. Connecting to the container..."
